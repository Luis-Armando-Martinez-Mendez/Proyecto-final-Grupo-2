...
namespace MonteCarloSim
{
    ...
    {
        // Pestaña 1: Líneas de Trayectoria
        public void DibujarTrayectorias(Chart chart, SimulationResults res, SimulationParameters param)
        {
            chart.Series.SuspendUpdates();
            chart.Series.Clear();

            // 1. Muestra de rutas (Azul)
            int rutasVisibles = Math.Min(150, param.NumeroSimulaciones);
            for (int i = 0; i < rutasVisibles; i++)
            {
                Series s = new Series { ChartType = SeriesChartType.FastLine, Color = Color.FromArgb(30, Color.DodgerBlue), IsVisibleInLegend = false };
                int paso = param.DiasTotales > 300 ? 5 : 1;
                for (int d = 0; d < param.DiasTotales; d += paso) s.Points.AddXY(d, res.Trayectorias[i][d]);
                chart.Series.Add(s);
            }
        ...
        {
            chart.Series.Clear();
            Series sHist = new Series("Frecuencia") { ChartType = SeriesChartType.Column, Color = Color.CornflowerBlue };
        ...
        public void DibujarPastel(Chart chart, double[] preciosFinales, double capitalInicial)
        {
            ...
            Series sPie = new Series("Probabilidad") { ChartType = SeriesChartType.Pie };
        ...
        public void DibujarBoxPlot(Chart chart, double[] preciosFinales)
        {
            ...
            Series sBox = new Series("Rango") { ChartType = SeriesChartType.BoxPlot };
